...
# Keyed by spreadsheet.id — DPL and AMB (and any future company) each get
# their own independently-cached Beneficiary Master, since both live in
# the same process (web_app.py calls run_pipeline() in-process, so one
# worker can handle a DPL upload and an AMB upload back to back). A flat,
# un-keyed cache would let whichever company loaded first silently leak
# into the other's classification for the rest of that process's life.
_beneficiary_cache_by_sheet: dict[str, dict[str, str]] = {}
_beneficiary_conflict_cache_by_sheet: dict[str, dict[str, list[str]]] = {}
_beneficiary_secondary_heads_cache_by_sheet: dict[str, dict[str, list[str]]] = {}
...
_beneficiary_conflict_head1_cache_by_sheet: dict[str, dict[str, str]] = {}
...
def _load_beneficiary_cache(spreadsheet: Optional[gspread.Spreadsheet]) -> dict[str, str]:
    ...
    process per spreadsheet, keeping only STATUS="Confirmed" rows.
    ...
    classification.
    """
    key = spreadsheet.id if spreadsheet is not None else ""
    if key in _beneficiary_cache_by_sheet:
        return _beneficiary_cache_by_sheet[key]

    beneficiary_cache: dict[str, str] = {}
    conflict_cache: dict[str, list[str]] = {}
    secondary_heads_cache: dict[str, list[str]] = {}
    conflict_head1_cache: dict[str, str] = {}
    _beneficiary_cache_by_sheet[key] = beneficiary_cache
    _beneficiary_conflict_cache_by_sheet[key] = conflict_cache
    _beneficiary_secondary_heads_cache_by_sheet[key] = secondary_heads_cache
    _beneficiary_conflict_head1_cache_by_sheet[key] = conflict_head1_cache
    if spreadsheet is None:
        return beneficiary_cache
    ...
    for row in rows[1:]:
        ...
        if status == _BENEFICIARY_MASTER_STATUS_CONFLICT:
            if name and head:
                ...
                conflict_head1_cache[name] = head
            for i in (h2i, h3i):
        ...
        if name and head:
            ...
            if secondary:
                secondary_heads_cache[name] = secondary

    for name, heads in conflict_heads_by_name.items():
        conflict_cache[name] = sorted(heads)
...
def _lookup_beneficiary_conflict(
    ...
    if not name:
        return None
    _load_beneficiary_cache(spreadsheet)  # populates _beneficiary_conflict_cache_by_sheet too
    key = spreadsheet.id if spreadsheet is not None else ""
    return _beneficiary_conflict_cache_by_sheet.get(key, {}).get(name)
...
def _lookup_beneficiary_conflict_head1(
    ...
    if not name:
        return None
    _load_beneficiary_cache(spreadsheet)  # populates _beneficiary_conflict_head1_cache_by_sheet too
    key = spreadsheet.id if spreadsheet is not None else ""
    return _beneficiary_conflict_head1_cache_by_sheet.get(key, {}).get(name)
...
def _lookup_beneficiary_secondary_heads(
    ...
    if not name:
        return None
    _load_beneficiary_cache(spreadsheet)  # populates _beneficiary_secondary_heads_cache_by_sheet too
    key = spreadsheet.id if spreadsheet is not None else ""
    return _beneficiary_secondary_heads_cache_by_sheet.get(key, {}).get(name)
